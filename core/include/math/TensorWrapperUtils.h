...
class TensorWrapperUtils {
    ...
    TensorWrapper<T> concatenate(const std::vector<TensorWrapper<T>>& tensors) {
        if (tensors.empty()) {
            throw std::invalid_argument(
                "concatenate: tensors vector cannot be empty");
        }
     ...
     * and distributes its data back into the original tensor structures. The
     * function writes data directly to the target tensors' underlying storage.
     *
     * Example:
     * @code
    ...
    void distributeTensors(const TensorWrapper<T>& flatTensor,
        ...
        if (tensors.size() != sizes.size()) {
            throw std::invalid_argument("distributeTensors: tensors and sizes "
        ...
        if (totalSize != flatTensor.getTotalSize()) {
            throw std::invalid_argument(
                "distributeTensors: total size of tensors ("
    ...
    template <typename T>
    TensorWrapper<T> random(TensorShape shape, T minValue = 0, T maxValue = 1) {
        TensorWrapper<T> result(shape);
        ...
        for (int i = 0; i < result.getTotalSize(); ++i) {
            result.getRawData()[i] = dist(engine);
        }
