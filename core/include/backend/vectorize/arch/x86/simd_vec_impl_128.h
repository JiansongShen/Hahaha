...
template <> struct SimdVec<f32, 128> {
...
template <> struct SimdVec<f64, 128> {
...
template <> struct SimdVec<i32, 128> {
    ...
    SimdVec& operator*=(SimdVec const& other) {
        v_ = _mm_mullo_epi32(v_, other.v_);
        return *this;
    }
#endif
    SimdVec& operator+=(SimdVec const& other) {
...
template <> struct SimdVec<i64, 128> {
