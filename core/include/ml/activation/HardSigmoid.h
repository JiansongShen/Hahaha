...
template <typename T> class HardSigmoid : public Activation<T> {
  public:
     ...
     * @return std::shared_ptr<ComputeNode<T>> The output compute node with hard sigmoid applied.
     */
 ...
 * @return std::shared_ptr<ComputeNode<T>> The output compute node with hard sigmoid applied.
 */
template <typename T> std::shared_ptr<ComputeNode<T>> hardSigmoid(std::shared_ptr<ComputeNode<T>> input);
