...
TEST_F(TopoSortTest, LinearGraph) {
    ...
    auto b = std::make_shared<ComputeNode<float>>(
        a, a, bData, hahaha::common::Operator::Add, nullptr); // Dummy op

    auto cData = std::make_shared<TensorWrapper<float>>(TensorShape({1}), 3.0f);
    auto c = std::make_shared<ComputeNode<float>>(
        b, b, cData, hahaha::common::Operator::Add, nullptr);
...
TEST_F(TopoSortTest, DiamondGraph) {
    ...
    */
    ...
    auto bData = std::make_shared<TensorWrapper<float>>(TensorShape({1}), 2.0f);
    auto b = std::make_shared<ComputeNode<float>>(
        a, a, bData, hahaha::common::Operator::Add, nullptr);

    auto cData = std::make_shared<TensorWrapper<float>>(TensorShape({1}), 3.0f);
    auto c = std::make_shared<ComputeNode<float>>(
        a, a, cData, hahaha::common::Operator::Add, nullptr);

    auto dData = std::make_shared<TensorWrapper<float>>(TensorShape({1}), 4.0f);
    auto d = std::make_shared<ComputeNode<float>>(
        b, c, dData, hahaha::common::Operator::Add, nullptr);
...
TEST_F(TopoSortTest, SharedDependency) {
    ...
    */
    ...
    auto cData = std::make_shared<TensorWrapper<float>>(TensorShape({1}), 3.0f);
    auto c = std::make_shared<ComputeNode<float>>(
        a, b, cData, hahaha::common::Operator::Add, nullptr);
...
TEST_F(TopoSortTest, UnaryAndBinaryMix) {
    ...
    */
    ...
    auto bData = std::make_shared<TensorWrapper<float>>(TensorShape({1}), 2.0f);
    auto b = ComputeNode<float>::createUnary(
        a, bData, hahaha::common::Operator::Add, nullptr);
