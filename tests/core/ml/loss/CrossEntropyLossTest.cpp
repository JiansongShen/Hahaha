...
TYPED_TEST(CrossEntropyLossTypedTest, ScalarInputs_OneHotEncoded_ComputesCorrectly) {
    ...
    auto lossNode = hahaha::ml::computeCrossEntropyLoss(yTrueNode, yPredNode);
...
TYPED_TEST(CrossEntropyLossTypedTest, VectorInputs_OneHotEncoded_ComputesSumCorrectly) {
    using T = typename TestFixture::Type;
    auto yTrueData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(1), static_cast<T>(0), static_cast<T>(0)});
    auto yPredData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(0.9), static_cast<T>(0.05), static_cast<T>(0.05)});
    auto yTrueNode = std::make_shared<ComputeNode<T>>(yTrueData);
    auto yPredNode = std::make_shared<ComputeNode<T>>(yPredData);
    auto lossNode = hahaha::ml::computeCrossEntropyLoss(yTrueNode, yPredNode);
    auto lossData = lossNode->getData();
...
TYPED_TEST(CrossEntropyLossTypedTest, PerfectPrediction_ReturnsSmallValue) {
    using T = typename TestFixture::Type;
    auto yTrueData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(1), static_cast<T>(0)});
    auto yPredData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(0.999999), static_cast<T>(0.000001)});
    auto yTrueNode = std::make_shared<ComputeNode<T>>(yTrueData);
    auto yPredNode = std::make_shared<ComputeNode<T>>(yPredData);
    auto lossNode = hahaha::ml::computeCrossEntropyLoss(yTrueNode, yPredNode);
    auto lossData = lossNode->getData();
...
TYPED_TEST(CrossEntropyLossTypedTest, ZeroPrediction_HandlesEpsilon) {
    using T = typename TestFixture::Type;
    auto yTrueData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(1), static_cast<T>(0)});
    auto yPredData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(0), static_cast<T>(1)});
    auto yTrueNode = std::make_shared<ComputeNode<T>>(yTrueData);
    auto yPredNode = std::make_shared<ComputeNode<T>>(yPredData);
    auto lossNode = hahaha::ml::computeCrossEntropyLoss(yTrueNode, yPredNode);
    auto lossData = lossNode->getData();
...
TYPED_TEST(CrossEntropyLossTypedTest, MultipleClasses_OneHotEncoded_ComputesCorrectly) {
    using T = typename TestFixture::Type;
    auto yTrueData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(0), static_cast<T>(1), static_cast<T>(0)});
    auto yPredData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(0.1), static_cast<T>(0.7), static_cast<T>(0.2)});
    auto yTrueNode = std::make_shared<ComputeNode<T>>(yTrueData);
    auto yPredNode = std::make_shared<ComputeNode<T>>(yPredData);
    auto lossNode = hahaha::ml::computeCrossEntropyLoss(yTrueNode, yPredNode);
    auto lossData = lossNode->getData();
...
TYPED_TEST(CrossEntropyLossTypedTest, SoftTargets_NonOneHot_ComputesCorrectly) {
    using T = typename TestFixture::Type;
    auto yTrueData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(0.8), static_cast<T>(0.2)});
    auto yPredData = std::make_shared<TensorWrapper<T>>(NestedData<T>{static_cast<T>(0.7), static_cast<T>(0.3)});
    auto yTrueNode = std::make_shared<ComputeNode<T>>(yTrueData);
    auto yPredNode = std::make_shared<ComputeNode<T>>(yPredData);
    auto lossNode = hahaha::ml::computeCrossEntropyLoss(yTrueNode, yPredNode);
    auto lossData = lossNode->getData();
...
TEST(CrossEntropyLossTest, CrossEntropyLossClass_MatchesConvenienceFunction) {
    ...
    auto yTrueData = std::make_shared<TensorWrapper<f32>>(NestedData<f32>{1.0f, 0.0f, 0.0f});
    auto yPredData = std::make_shared<TensorWrapper<f32>>(NestedData<f32>{0.9f, 0.05f, 0.05f});
    ...
    auto loss2Node = hahaha::ml::computeCrossEntropyLoss(yTrueNode, yPredNode);
