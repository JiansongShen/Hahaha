...
TEST(MSELossTest, ComputeMSELoss_ZeroDimension_Scalar) {
    // 0D scalar inputs
    Tensor<float> yTrue(NestedData<float>(3.0f)); // Scalar tensor
    Tensor<float> yPred(NestedData<float>(1.0f));
    auto loss = hahaha::ml::computeMSELoss(yTrue, yPred);
    // (3-1)^2 = 4
    EXPECT_FLOAT_EQ(loss.at({}), 4.0f);
...
TEST(MSELossTest, ComputeMSELoss_TwoDimension_Matrix) {
    // 2D matrix inputs
    Tensor<float> yTrue(NestedData<float>{{1.0f, 2.0f}, {3.0f, 4.0f}});
    Tensor<float> yPred(NestedData<float>{{2.0f, 1.0f}, {2.0f, 5.0f}});
    auto loss = hahaha::ml::computeMSELoss(yTrue, yPred);
    // (1-2)^2 + (2-1)^2 + (3-2)^2 + (4-5)^2 = 1 + 1 + 1 + 1 = 4
    EXPECT_FLOAT_EQ(loss.at({}), 4.0f);
}

TEST(MSELossTest, ComputeMSELoss_ThreeDimension_Tensor) {
    // 3D tensor inputs
    Tensor<float> yTrue(NestedData<float>{{{1.0f, 2.0f}, {3.0f, 4.0f}},
                                          {{5.0f, 6.0f}, {7.0f, 8.0f}}});
    Tensor<float> yPred(NestedData<float>{{{2.0f, 1.0f}, {2.0f, 5.0f}},
...
TEST(MSELossTest, ComputeMSELoss_ScalarTypes_Distinction) {
    ...
    // 0D scalar
    Tensor<float> yTrue0(NestedData<float>(3.0f));
    Tensor<float> yPred0(NestedData<float>(1.0f));
    auto loss0 = hahaha::ml::computeMSELoss(yTrue0, yPred0);

    // 1D scalar [1]
    Tensor<float> yTrue1(NestedData<float>{3.0f});
    Tensor<float> yPred1(NestedData<float>{1.0f});
    auto loss1 = hahaha::ml::computeMSELoss(yTrue1, yPred1);

    // 2D scalar [1, 1]
    Tensor<float> yTrue2(NestedData<float>{{3.0f}});
    Tensor<float> yPred2(NestedData<float>{{1.0f}});
    auto loss2 = hahaha::ml::computeMSELoss(yTrue2, yPred2);

    // 3D scalar [1, 1, 1]
    Tensor<float> yTrue3(NestedData<float>{{{3.0f}}});
    Tensor<float> yPred3(NestedData<float>{{{1.0f}}});
