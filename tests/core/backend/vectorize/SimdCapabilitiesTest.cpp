...
TEST(SimdCapabilitiesX86, CompileTimeRuntimeConsistency) {
    ...
    if (SimdCapabilitiesCompileTime::kSSE2) {
        EXPECT_TRUE(rt.sse2)
            << "Compile-time SSE2 enabled but runtime detection failed";
    }

    if (SimdCapabilitiesCompileTime::kSSE41) {
        EXPECT_TRUE(rt.sse41)
            << "Compile-time SSE4.1 enabled but runtime detection failed";
    }

    if (SimdCapabilitiesCompileTime::kAVX) {
        EXPECT_TRUE(rt.avx)
            << "Compile-time AVX enabled but runtime detection failed";
    }

    if (SimdCapabilitiesCompileTime::kAVX2) {
        EXPECT_TRUE(rt.avx2)
            << "Compile-time AVX2 enabled but runtime detection failed";
    }

    if (SimdCapabilitiesCompileTime::kAVX512F) {
        EXPECT_TRUE(rt.avx512f)
            << "Compile-time AVX-512F enabled but runtime detection failed";
    }
}
